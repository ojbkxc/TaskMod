...
│           │   ├── TtsManager.kt      # 原生 TTS（借鉴 Agora，多引擎+看门狗+诊断+网络 TTS）
│           │   ├── TtsReceiver.kt     # TTS 广播接收器（供 server am broadcast 调用，注入网络 TTS 配置）
...
### ✅ 已完成
- **网络 TTS（OpenAI 兼容 /audio/speech）+ TTS barge-in 打断**（2026-08-20，本次会话）：借鉴 Agora `TtsManager.kt` 的网络 TTS 能力到 TaskMod APK 端 + Rust 服务端 barge-in。(1) `TtsManager.kt`（333→511 行，+178）：新增 `NetworkTtsConfig` data class + `speakNetwork()`/`synthesizeNetSpeech()`/`playNetAudio()`/`normalizeSpeechUrl()`，`speak()` 入口先检查 `networkTtsConfig` 非 null 走网络 TTS（OpenAI 兼容 `POST /audio/speech`，MediaPlayer 播放），`stop()`/`shutdown()` 清理 `netPlayer`。(2) `ConfigManager.kt`（96→108 行，+12）：`AppConfig` 加 `networkTtsEnabled`/`networkTtsBaseUrl`/`networkTtsApiKey`/`networkTtsModel`/`networkTtsVoice` 字段 + `isNetworkTtsConfigured()`。(3) `TtsReceiver.kt`（54→67 行，+13）：`handleSpeak()` 每次从 `ConfigManager.load()` 注入网络 TTS 配置（支持运行时切换）。(4) `tts.rs`（879→899 行，+20）：`AtomicU64` generation counter 实现 barge-in，`speak()` 新请求递增 generation 取消旧播放循环，`stop_tts()` 同理递增，循环内检测 generation 变化提前退出。CI 全绿验证通过（run 32324609169，commit `f620e13`：build-server/build-apk/lint 均 success）。期间修复 3 次 `cargo fmt` 格式问题（`tracing::info!` 多行格式 + 多余空行）。
...
- [x] **网络 TTS + barge-in 打断**：APK 端借鉴 Agora `NetworkTtsConfig` + `speakNetwork`/`synthesizeNetSpeech`/`playNetAudio`（OpenAI 兼容 `/audio/speech`），Rust 服务端 `AtomicU64` generation counter 实现 barge-in。commit `f620e13`，CI 全绿（run 32324609169）。
...
- **2026-08-20**：借鉴 Agora 网络 TTS + barge-in 到 TaskMod（commit `f620e13`，CI run 32324609169 全绿）。**APK 端**（借鉴 Agora `TtsManager.kt`）：(1) `TtsManager.kt`（333→511 行）新增 `NetworkTtsConfig` data class + `speakNetwork()`/`synthesizeNetSpeech()`/`playNetAudio()`/`normalizeSpeechUrl()`，`speak()` 入口先检查 `networkTtsConfig` 非 null 走网络 TTS（OpenAI 兼容 `POST /audio/speech`，MediaPlayer + AudioAttributes USAGE_MEDIA 播放），`stop()`/`shutdown()` 清理 `netPlayer`；(2) `ConfigManager.kt`（96→108 行）`AppConfig` 加 `networkTtsEnabled`/`networkTtsBaseUrl`/`networkTtsApiKey`/`networkTtsModel`/`networkTtsVoice` 字段 + `isNetworkTtsConfigured()`；(3) `TtsReceiver.kt`（54→67 行）`handleSpeak()` 每次从 `ConfigManager.load()` 注入网络 TTS 配置（支持运行时切换）。**Rust 服务端**：`tts.rs`（879→899 行）`AtomicU64` generation counter 实现 barge-in，`speak()` 新请求递增 generation 取消旧播放循环，`stop_tts()` 同理递增，循环内检测 generation 变化提前退出。期间修复 3 次 `cargo fmt` 格式问题（`tracing::info!` 多行格式 + 多余空行）。"防止 TTS 音频回声"不适用（TaskMod 无语音对话/麦克风采集功能）。
