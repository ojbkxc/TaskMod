...
impl AiTool for ListTasksTool {
    ...
    fn description(&self) -> &str { "查看所有定时任务列表，返回每个任务的ID、时间、星期、脚本名、类型" }
        ...
        Box::pin(async {
            ...
            for (i, (time, weeks, script, task_type, interval)) in tasks.iter().enumerate() {
                result.push_str(&format!(
                    "ID:{} | 时间:{} | 星期:{} | 脚本:{} | 类型:{}",
                    i + 1, time, weeks, script, task_type
                ));
                if let Some(iv) = interval {
...
impl AiTool for AddTaskTool {
    ...
    fn description(&self) -> &str { "添加新的定时任务。time格式如 08:30，weeks用逗号分隔如 1,2,3,4,5（1=周一）或 * 表示每天，task_type可选 daily/interval/weekly" }
    fn parameters(&self) -> serde_json::Value {
        json!({
            "type": "object",
            "properties": {
    ...
    fn execute(&self, args: &str) -> BoxFuture<'_, String> {
        ...
        Box::pin(async move {
            ...
            tasks.push((time.clone(), weeks.clone(), script.clone(), task_type.clone(), interval));
            match write_tasks(&tasks) {
                Ok(()) => format!("任务已添加：{} {} {} [{}] 30秒内自动生效", time, weeks, script, task_type),
                Err(e) => e,
            }
        })
    }
...
impl AiTool for DeleteTaskTool {
    ...
    fn description(&self) -> &str { "根据ID删除定时任务，ID从1开始" }
    fn parameters(&self) -> serde_json::Value {
        json!({
            "type": "object",
            "properties": {
    ...
    fn execute(&self, args: &str) -> BoxFuture<'_, String> {
        ...
        Box::pin(async move {
            ...
            match write_tasks(&tasks) {
                Ok(()) => format!("已删除任务ID{}: {} {} {}", id, removed.0, removed.1, removed.2),
                Err(e) => e,
            }
        })
    }
...
impl AiTool for ModifyTaskTool {
    fn name(&self) -> &str { "modify_task" }
    fn description(&self) -> &str { "修改指定ID的定时任务，只需传入要修改的字段" }
    fn parameters(&self) -> serde_json::Value {
        json!({
            "type": "object",
            "properties": {
...
impl AiTool for ListScriptsForTaskTool {
    fn name(&self) -> &str { "list_available_scripts" }
    fn description(&self) -> &str { "列出所有可用于定时任务的脚本文件" }
    fn parameters(&self) -> serde_json::Value {
        json!({"type": "object", "properties": {}})
    }
    fn execute(&self, _args: &str) -> BoxFuture<'_, String> {
